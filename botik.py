...
def get_crash_point(m):
 r=random.random()
 if m<=1.5:return round(random.uniform(m,m*2.5),2)if r<0.8 else round(random.uniform(1.0,m),2)
 elif m<=2:return round(random.uniform(m,m*2.2),2)if r<0.65 else round(random.uniform(1.0,m),2)
 elif m<=3:return round(random.uniform(m,m*2.0),2)if r<0.45 else round(random.uniform(1.0,m),2)
 elif m<=5:return round(random.uniform(m,m*1.8),2)if r<0.25 else round(random.uniform(1.0,m),2)
 elif m<=10:return round(random.uniform(m,m*1.5),2)if r<0.10 else round(random.uniform(1.0,m),2)
 elif m<=20:return round(random.uniform(m,m*1.3),2)if r<0.04 else round(random.uniform(1.0,m),2)
 elif m<=50:return round(random.uniform(m,m*1.2),2)if r<0.01 else round(random.uniform(1.0,m),2)
 else:return round(random.uniform(m,m*1.1),2)if r<0.002 else round(random.uniform(1.0,m),2)
...
async def start(u,c):
 ...
 if c.args and len(c.args)>0 and c.args[0].startswith("ref_"):
  ...
  if ref_id!=str(u.effective_user.id)and not d.get("referrer"):
   ...
   await u.message.reply_text(f"🎉 Вы по реферальной ссылке!\n💰 +{REF_BONUS} lkoin\n👤 Реферер +{REF_BONUS} lkoin")
   return
 save()
 await u.message.reply_text(f"👋 Привет!\n🔗 Твоя ссылка: {ref_link}\n💎 За друга {REF_BONUS} lkoin\n👥 Приглашено: {d.get('refs',0)}\n📋 'команды' - список")

async def commands_list(u,c):
 await u.message.reply_text("📋 КОМАНДЫ\n\n🎮 ИГРЫ:\nкраш <ставка> <множитель>\nкраш всё\nбашня <ставка>\nлотерея\nслоты <ставка>\n\n💰 ФИНАНСЫ:\nбаланс / б / бал\nпоинт - LMP\nбонус\nдонат <⭐>\n\n🔗 РЕФЕРАЛКА:\nреф - ссылка\nрефы - статистика\n\n🏪 ОБМЕННИК:\nобменник - продажа LMP\n\n📊 ИНФО:\nигры\nтоп")

async def ref_link(u,c):
 bot_name=(await c.bot.get_me()).username
 await u.message.reply_text(f"🔗 https://t.me/{bot_name}?start=ref_{u.effective_user.id}\n💎 За друга {REF_BONUS} lkoin")

async def ref_stats(u,c):
 d=get_user(u.effective_user.id)
 await u.message.reply_text(f"👥 {d.get('refs',0)} чел\n💎 {d.get('refs',0)*REF_BONUS} lkoin")

async def games(u,c):
 await u.message.reply_text(f"🎮 ИГРЫ\n\n1️⃣ КРАШ - краш <ставка> <множитель>\n   Шансы: до 2x-65%, до 5x-25%, до 10x-10%, до 20x-4%, до 50x-1%, до 100x-0.2%\n\n2️⃣ БАШНЯ - башня <ставка>\n   7 уровней: 1.3x→1.8x→2.6x→3.8x→5.5x→8.0x→12.0x\n\n3️⃣ ЛОТЕРЕЯ - лотерея\n   Приз 1000 lkoin, перезарядка 6ч\n\n4️⃣ СЛОТЫ - слоты <ставка>\n   Выпади 777 - x5, три одинаковых - x2\n\n💎 1 LMP = {LMP_PRICE} lkoin")

async def bonus(u,c):
 d=get_user(u.effective_user.id)
 if is_owner(u.effective_user.id):return await u.message.reply_text(f"🎁 +{random.randint(30,500)} lkoin!\n💰 0")
 now=int(time.time())
 if d.get("bonus",0)and(now-d["bonus"])<3600:return await u.message.reply_text(f"⏳ Через {(3600-(now-d['bonus']))//60} мин.")
...
async def exchanger(u,c):
 d=get_user(u.effective_user.id);lmp=d.get("lmp",0);balance=d.get("balance",0)
 kb=[]
 if lmp>=1:
  kb.append([InlineKeyboardButton(f"💰 Обменять 1 LMP = {LMP_PRICE} lkoin",callback_data=f"sell_1_{u.effective_user.id}")])
 if lmp>=5:
  kb.append([InlineKeyboardButton(f"💰 Обменять 5 LMP = {LMP_PRICE*5} lkoin",callback_data=f"sell_5_{u.effective_user.id}")])
 if lmp>=10:
  kb.append([InlineKeyboardButton(f"💰 Обменять 10 LMP = {LMP_PRICE*10} lkoin",callback_data=f"sell_10_{u.effective_user.id}")])
 if lmp>0:
  kb.append([InlineKeyboardButton(f"💰 Обменять всё ({lmp} LMP = {lmp*LMP_PRICE} lkoin)",callback_data=f"sell_all_{u.effective_user.id}")])
 if not kb:
  kb.append([InlineKeyboardButton("❌ Нет LMP для обмена",callback_data="none")])
 await u.message.reply_text(
  f"🏪 ОБМЕННИК LMP → lkoin\n\n"
  f"💎 У тебя: {lmp} LMP\n"
  f"💰 lkoin: {balance} lkoin\n"
  f"📌 Курс: 1 LMP = {LMP_PRICE} lkoin\n\n"
  f"Выбери сколько обменять:",
...
async def sell_callback(update,context):
 ...
 if data.startswith("sell_"):
  ...
  earned=a*LMP_PRICE
  kb=[[InlineKeyboardButton("✅ Подтвердить",callback_data=f"cf_{uid}_{a}"),InlineKeyboardButton("❌ Отменить",callback_data=f"cn_{uid}")]]
  await q.edit_message_text(
   f"💎 ОБМЕН LMP → lkoin\n\n"
   f"Ты обмениваешь: {a} LMP\n"
   f"Получишь: {fmt(earned)} lkoin\n"
   f"Курс: 1 LMP = {LMP_PRICE} lkoin\n\n"
...
async def confirm_callback(update,context):
 ...
 if data.startswith("cf_"):
  ...
  await q.edit_message_text(
   f"✅ ОБМЕН ВЫПОЛНЕН!\n\n"
   f"Обменяно: {a} LMP\n"
   f"Получено: +{fmt(earned)} lkoin\n\n"
   f"💰 Баланс: {d.get('balance',0)} lkoin\n"
   f"💎 LMP: {d.get('lmp',0)} LMP"
  )
  await q.answer()
...
async def cancel_callback(update,context):
 ...
 if data.startswith("cn_"):
  ...
  if q.from_user.id!=uid:return await q.answer("❌ Не твоя!")
  await q.edit_message_text("❌ Обмен отменен!");await q.answer()
...
async def slots(u,c):
 ...
 player_num=random.randint(1,999)
 
 # Проверяем выигрыш
 win_mult=0
 if tuple(result)==("7⃣","7⃣","7⃣"):
  win_mult=5
 elif tuple(result) in SLOT_PAYOUTS:
  win_mult=SLOT_PAYOUTS[tuple(result)]
 ...
 if win_mult>0:
  ...
  await u.message.reply_text(
   ...
   f"💥 Слоты · Выигрыш!\n"
   f"·····················\n"
   f"💸 Ставка: {fmt(bet)} lkoin\n"
   f"🏆 Выигрыш: +{fmt(win)} lkoin (x{win_mult})\n"
   f"🔮 Выпало: {result[0]}{result[1]}{result[2]}"
  )
 else:
...
async def show_tower(update,context,gid,is_callback):
 ...
 for r in range(g["rows"]):
  ...
  for i in range(5):
   k=f"{r}_{i}"
   if k in g["revealed"]:
    row.append(InlineKeyboardButton("💣"if g["bomb"]==i and r==g["rows"]-1 else"✅",callback_data=f"tn_{gid}"))
   else:row.append(InlineKeyboardButton("❓",callback_data=f"tc_{gid}_{r}_{i}"))
  kb.append(row)
 if g["level"]>0:
...
async def tower_cb(update,context):
 ...
 if data.startswith("tc_"):
  ...
  g=TOWER_GAMES.get(gid)
  if not g or g["user_id"]!=q.from_user.id:return await q.answer("❌ Не твоя!")
  k=f"{r}_{c}"
  if k in g["revealed"]:return await q.answer("❌ Уже открыто!")
  g["revealed"].append(k)
...
async def admin(u,c):
 ...
 cmd=c.args[0].lower()
 if cmd=="статистика":
  await u.message.reply_text(f"📊 СТАТИСТИКА\nПользователей: {len(users)}\nlkoin: {fmt(sum(d.get('balance',0)for d in users.values()))}\nLMP: {fmt(sum(d.get('lmp',0)for d in users.values()))}\nИгр: {sum(d.get('games',0)for d in users.values())}\nРефов: {sum(d.get('refs',0)for d in users.values())}")
 elif cmd=="выдать"and len(c.args)==3:
  d=get_user(c.args[1]);d["balance"]=d.get("balance",0)+int(c.args[2]);save()
  await u.message.reply_text(f"✅ Выдано {c.args[2]} lkoin {c.args[1]}")
 elif cmd=="выдать_lmp"and len(c.args)==3:
  d=get_user(c.args[1]);d["lmp"]=d.get("lmp",0)+int(c.args[2]);save()
  await u.message.reply_text(f"✅ Выдано {c.args[2]} LMP {c.args[1]}")
 elif cmd=="забрать"and len(c.args)==3:
  d=get_user(c.args[1])
  if d.get("balance",0)<int(c.args[2]):return await u.message.reply_text("❌ Мало lkoin!")
  d["balance"]=d.get("balance",0)-int(c.args[2]);save()
  await u.message.reply_text(f"✅ Забрано {c.args[2]} lkoin у {c.args[1]}")
 elif cmd=="сброс"and len(c.args)==2:
  if c.args[1]in users:del users[c.args[1]];save();await u.message.reply_text(f"✅ {c.args[1]} удален")
  else:await u.message.reply_text("❌ Не найден")
 elif cmd=="обнулить":
  for uid in users:users[uid]["balance"]=0;users[uid]["bonus"]=0;users[uid]["lottery"]=0;users[uid]["lmp"]=0;users[uid]["games"]=0;users[uid]["losses"]=0;users[uid]["refs"]=0
  save();await u.message.reply_text(f"✅ Обнулено {len(users)} пользователей!")
 elif cmd=="рассылка"and len(c.args)>=2:
  t=" ".join(c.args[1:]);count=0
  for uid in users:
   try:await u.bot.send_message(chat_id=int(uid),text=f"📢 {t}");count+=1;time.sleep(0.05)
   except:pass
  await u.message.reply_text(f"✅ Отправлено {count}")
async def text(u,c):
